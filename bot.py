...
# РАСШИРЕННЫЙ СЛОВАРЬ (Исправленная структура)
LESSONS = {
...
ALPHABET_RU = "А [A], Б [B], В [V], Г [G], Д [D], Е [Ye], Ё [Yo], Ж [Zh], З [Z], И [Ee], Й [Y], К [K], Л [L], М [M], Н [N], О [O], П [P], Р [R], С [S], T [T], У [U], Ф [F], Х [Kh], Ц [Ts], Ч [Ch], Ш [Sh], Щ [Shch], Ъ [-], Ы [Y], Ь ['], Э [E], Ю [Yu], Я [Ya]"
...
async def cmd_start(message: types.Message):
    ...
    builder.adjust(1)
    await message.answer(f"Привет, {message.from_user.first_name}! 👋\nНачнем обучение! / Let's start!", reply_markup=builder.as_markup(resize_keyboard=True))
